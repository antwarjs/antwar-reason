...
describe("Transform sections", () => {
  it("transforms a root section", () => {
    ...
    const parsedPages = parseSectionPages(sectionName, section, context(path => [`./${path}`]));
  ...
  it("transforms a nested page", () => {
    ...
    const parsedPages = parseSectionPages(sectionName, section, context(path => [`./${path}`], file => `nested/page/${file}`));
    const result = transformSections(sectionName, section, parsedPages);
    const expected = [{
      type: "page",
  ...
  it("transforms a child section", () => {
    ...
    const parsedPages = parseSectionPages(sectionName, section, context(path => [`./docs/${path}`]));
    const result = transformSections(sectionName, section, parsedPages);
    const expected = [{
      type: "page",
  ...
  it("transforms root and child section", () => {
    ...
    const parsedPages = parseSectionPages(sectionName, section, context(path => [`./${path}`, `./docs/${path}`]));
    const result = transformSections(sectionName, section, parsedPages);
    const expected = [{
      type: "page",
      fileName: "first.md",
...
function context(shapePath, shapeFile = id) {
  ...
  const modules = _.fromPairs(_.flatMap(files.map(({ file, sort }) => ({
    file: shapeFile(file),
    sort
