...
module.exports = function parseSectionPages(sectionName, section, modules) {
  ...
  const ret = _.map(moduleKeys, name => {
    ...
    const nearestSectionName = Object.keys(section.paths || {}).filter(path => fileNameWithoutExtension.startsWith(path)).sort()[0] || sectionName;
    ...
    if (_path.basename(fileNameWithoutExtension).endsWith("index")) {
      return {
        ...
        url: fileNameWithoutExtension === "index" && sectionName === "/" ? "/" : joinUrl(sectionName, fileNameWithoutExtension.split("/index").slice(0, -1).join(""))
      };
    }
  ...
  const indexPages = _.map(Object.keys(section && section.paths || {}), childSectionName => {
